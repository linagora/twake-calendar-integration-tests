...
public interface TechnicalTokenService {
    ...
    record TechnicalTokenInfo(String domainId,
    ...
    class Impl implements TechnicalTokenService {
        ...
        public static final Function<String, Map<String, Object>> TOKEN_DATA_FUNCTION = domainId -> Map.of(
        ...
        @Override
        public Mono<JwtToken> generate(String domainId) {
            return Mono.fromCallable(() -> JWT.create()
                    .withIssuer(ISSUER)
                    .withClaim(CLAIM_NAME_DOMAIN_ID, domainId)
        ...
        public Mono<TechnicalTokenInfo> claim(JwtToken token) {
            ...
            return Mono.fromCallable(() -> jwtVerifier.verify(token.value()))
                .map(decodedJWT -> {
                    ...
                    return new TechnicalTokenInfo(domainId, data);
                })
                .subscribeOn(Schedulers.parallel())
